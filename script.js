...
const mobileSpeedRange = document.getElementById('mobileSpeedRange');
const mobilePop = document.getElementById('mobilePop');
const mobileGen = document.getElementById('mobileGen');
...
function openSettingsModal() {
    ...
    mobileSpeedRange.value = simulationSpeed;
    updateUI(); // Sync stats
}
...
if (closePatternBtn) closePatternBtn.addEventListener('click', closePatternModal);
...
if (mobileSpeedRange) {
    mobileSpeedRange.addEventListener('input', (e) => {
        simulationSpeed = parseInt(e.target.value);
        frameInterval = 1000 / simulationSpeed;
        if (speedRange) speedRange.value = simulationSpeed; // Sync main slider
    });
}
...
canvas.addEventListener('touchstart', (e) => {
    ...
    if (e.touches.length === 1) {
        ...
        // Only trigger pointer down if not already pinching
        if (!isPinching) {
...
canvas.addEventListener('touchmove', (e) => {
    ...
    if (e.touches.length === 1 && !isPinching) {
        ...
        if (lastPinchDist > 0) {
            // 1. Pan (Move based on center point delta)
            const dx = cx - lastPinchCenter.x;
            const dy = cy - lastPinchCenter.y;
            offsetX += dx;
            offsetY += dy;

            // 2. Zoom (Scale based on distance ratio)
            ...
            // Adjust offset to keep the world point under the center stable during zoom
...
function updateUI() {
    if (popDisplay) popDisplay.textContent = liveCells.size;
    if (genDisplay) genDisplay.textContent = generation;

    // Sync mobile stats if visible
    if (settingsModal && settingsModal.style.display === 'block') {
        if (mobilePop) mobilePop.textContent = liveCells.size;
        if (mobileGen) mobileGen.textContent = generation;
    }
}
...
speedRange.addEventListener('input', (e) => {
    simulationSpeed = parseInt(e.target.value);
    frameInterval = 1000 / simulationSpeed;
    if (mobileSpeedRange) mobileSpeedRange.value = simulationSpeed;
});
